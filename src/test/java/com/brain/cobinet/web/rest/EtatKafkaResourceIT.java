package com.brain.cobinet.web.rest;
...
import com.brain.cobinet.config.KafkaProperties;
...
class EtatKafkaResourceIT {
    ...
    void setup() {
        ...
        EtatKafkaResource kafkaResource = new EtatKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/etat-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/etat-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
